...
function publicKeyPath() {
  return app.isPackaged
    ? path.join(process.resourcesPath, 'resources', 'public_key.pem')
    : path.join(app.getAppPath(), 'resources', 'public_key.pem');
}
...
// Читает PlayerDebugMode для тех же версий CSXS, что и enableCepDebug.
// Возвращает true, если режим включён для всех (значит баннер показывать не нужно).
function cepDebugEnabled() {
  return new Promise((resolve) => {
    const versions = [9, 10, 11, 12];
    let cmds;
    if (process.platform === 'darwin') {
      cmds = versions.map((v) => 'defaults read com.adobe.CSXS.' + v + ' PlayerDebugMode');
    } else if (process.platform === 'win32') {
      cmds = versions.map((v) => 'reg query "HKCU\\Software\\Adobe\\CSXS.' + v + '" /v PlayerDebugMode');
    } else {
      return resolve(false);
    }
    let done = 0, onCount = 0;
    cmds.forEach((c) => exec(c, (err, stdout) => {
      done++;
      const out = String(stdout || '');
      const on = process.platform === 'darwin'
        ? out.trim() === '1'
        : /PlayerDebugMode\s+REG_SZ\s+0x1\b/i.test(out) || /PlayerDebugMode\s+REG_SZ\s+1\b/i.test(out);
      if (on) onCount++;
      if (done === cmds.length) resolve(onCount === versions.length);
    }));
  });
}
...
app.whenReady().then(async () => {
  ...
  license.init(app.getPath('userData'), publicKeyPath());
  ...
  ipcMain.handle('script:install', async (event, { scriptId, hostId }) => {
    ...
    // ЗАЩИТА: устанавливаем только при действующей лицензии (сервер + привязка устройства).
    if (!(await license.hasValidLicense())) {
      return { ok: false, error: 'нужна активная лицензия — войдите и активируйте устройство', code: 'no_license' };
    }
  ...
  ipcMain.handle('cep:status', async () => ({ enabled: await cepDebugEnabled() }));
  ipcMain.handle('shell:open', (_e, url) => shell.openExternal(url));

  // ---- аккаунт и лицензия ----
  ipcMain.handle('auth:state', () => license.getState());
  ipcMain.handle('auth:signup', (_e, { email, password, key }) => license.signup(email, password, key));
  ipcMain.handle('auth:login', (_e, { email, password }) => license.login(email, password));
  ipcMain.handle('auth:redeem', (_e, { key }) => license.redeem(key));
  ipcMain.handle('auth:logout', () => license.logout());
  ipcMain.handle('license:ensure', () => license.ensureLicense());
  ipcMain.handle('license:devices', () => license.listDevices());
